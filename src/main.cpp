...
int main(int argc, char *argv[])
{
    // Graphics
    std::cout << "---------------------------" << std::endl;
    std::cout << "  GVAR Ingestor by Aang23" << std::endl;
    std::cout << "---------------------------" << std::endl;
    std::cout << std::endl;

    TCLAP::CmdLine cmd("GVAR Ingestor by Aang23", ' ', "1.0");
    ...
    else if (optionBasebandDemod.isSet())
    {
        ...
        std::ofstream outputBin("pdr.bin", std::ios::binary);

        // GVAR Demodulator
        GVARDemodulator demodulator(6e6, true);
    ...
    else
    {
        ...
        if (ingestorConfig.write_demod_bin)
            outputBin = std::ofstream("pdr.bin", std::ios::binary);

        // GVAR Demodulator and Decoder
        GVARDemodulator demodulator(ingestorConfig.samplerate, true);
        GVARDecoder decoder(ingestorConfig.data_directory);

        // SDR Stuff
        SoapySDR::Device *device = SoapySDR::Device::make(ingestorConfig.device);
	if(ingestorConfig.device == "driver=airspy")
